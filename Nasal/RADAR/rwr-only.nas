...
var RWR = {
	...
	scan: func {
		...
		#       ECM
		#       radar on/off
		if (!getprop("instrumentation/rwr/serviceable") or !getprop("fdm/jsbsim/systems/electrical/generic-pwr")) {
...
enable_tacobject = 0;
props.globals.getNode("sim/multiplay/generic/int[2]",1).setIntValue(1);# Jaguar has no radar, so put it on standby
...
var getRWRList = func {
	return jaguar_rwr.vector_aicontacts_threats;
}
